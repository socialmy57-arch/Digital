...
function App() {
  ...
  const initApp = async () => {
    try {
      ...
      // UPDATED FOR VERCEL: /api/telegram-auth
...
function Dashboard({ user, onBack }) {
  ...
  const loadDashboard = async () => {
    ...
    setMyNumbers(nums || [])
    const { data: wins } = await supabase.from('winners').select('*, lottery_rounds(*)').eq('user_id', user.id).eq('announced', true).order('announced_at', { ascending: false })
    setMyWins(wins || [])
  }
...
function AdminPanel({ user, onBack }) {
  ...
  const createRound = async () => {
    ...
    await supabase.from('lottery_rounds').insert({
      ...
      ticket_price: 300, created_by: user.telegram_id
    })
  ...
  const drawWinners = async (roundId) => {
    ...
    if (!numbers || numbers.length < 3) { alert('Need at least 3 paid numbers to draw 3 winners.'); return }
    const { data: existingWinners } = await supabase.from('winners').select('number').eq('round_id', roundId)
    ...
    if (available.length < 3) { alert('Not enough unique numbers for 3 winners (some already drawn).'); return }
    ...
    const inserts = winners.map((entry, index) => ({
      round_id: roundId, number: entry.number, user_id: entry.user_id,
    ...
    if (error) { alert('Error drawing winners: ' + error.message); return }
    alert(`Winners drawn:\n1st: Number ${winners[0].number}\n2nd: Number ${winners[1].number}\n3rd: Number ${winners[2].number}`)
    setPrize1('20000'); setPrize2('5000'); setPrize3('2000'); loadTabData()
  }
      ...
      {activeTab === 'rounds' && (
        <div>
          ...
          <div className="card">
            ...
            <button onClick={() => { const id = prompt('Enter Round ID to draw winners:'); if (id) drawWinners(id) }} className="btn-orange" style={{ marginTop: '15px' }}>🎲 Draw All Three Winners</button>
          </div>
        </div>
      )}
